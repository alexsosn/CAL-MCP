...
model_anchor = "@dataclass(frozen=True, slots=True)\nclass _TableLink:\n"
...
text = replace_once(text, model_anchor, models + model_anchor, "context models")

service_anchor = (
    "        return _kwic_result(canonical_key, KwicScopeKind.DIALECT, result.value, provenance)"
    "\n\n\ndef _parse_kwic_hits"
)
    ...
    async def kwic_full_context(
        ...
        *,
        ...
        normalized_sub = (
            ...
            else _validate_decimal_id(subtext_id, "subtext_id")
        )
...
text = replace_once(text, service_anchor, service_replacement, "service method")

parser_anchor = "def _parse_kwic_hits(response: CalResponse) -> tuple[KwicHit, ...]:\n"
parser_block = textwrap.dedent(
    ...
    def _validate_full_context_response_url(
        ...
        *,
        ...
        if set(query) != {"file", "sub", "cset", "target"}:
            raise ConcordanceParseError(
                "CAL full-context response has unexpected or missing selectors"
            )
        ...
        if sub_values is None or len(sub_values) != 1:
            raise ConcordanceParseError(
                "CAL full-context subtext parameter is missing or repeated"
            )
    ...
    def _parse_full_context_row(
        ...
        *,
        ...
        lexical = [
            ...
            if _is_path(link.href, "getlex.php")
        ]
        if not lexical:
            return None
        if len(row.cells) != 2:
        ...
        empty_positions = [
            index for index, item in enumerate(parsed_tokens) if not item[2]
        ]
        if empty_positions:
        ...
        comment_links = [
            link for link in coordinate_cell.links if _is_path(link.href, "comment.php")
        ]
        if len(comment_links) > 1 or len(comment_links) != len(coordinate_cell.links):
            raise ConcordanceParseError(
                "CAL full-context coordinate cell has conflicting links"
            )
        ...
        if comment_links:
            ...
            if set(query) != {"coord"}:
                raise ConcordanceParseError(
                    "CAL full-context comment link has unexpected selectors"
                )
            ...
            if comment_coordinate != coordinate or comment.text != display_coordinate:
                raise ConcordanceParseError(
                    "CAL full-context comment coordinate contradicts its row"
                )
    ...
    def _parse_full_context_lexical_link(
        ...
        if not word.isascii() or not word.isdecimal():
            raise ConcordanceParseError(
                "CAL full-context lexical word index is not decimal"
            )
...
text = replace_once(text, parser_anchor, parser_block + parser_anchor, "context parser")

serializer_anchor = "def _provenance_to_dict(item: ConcordanceProvenance) -> dict[str, object]:\n"
serializer = textwrap.dedent(
    '''\
...
text = replace_once(text, serializer_anchor, serializer + serializer_anchor, "line serializer")
text = replace_once(
    text,
    '    "KwicDialectRef",\n    "KwicHit",\n',
    '    "KwicDialectRef",\n'
...
text = path.read_text()
instruction_old = '        "dialect identifiers, and cal_kwic_dialect for one explicit dialect. Use "\n'
instruction_new = (
...
text = replace_once(text, instruction_old, instruction_new, "server instructions")
tool_anchor = textwrap.dedent(
    '''\
    @mcp.tool(
...
text = replace_once(text, tool_anchor, tool + tool_anchor, "server tool")
path.write_text(text)

path = Path("src/cal_mcp/release_surface.py")
text = path.read_text()
text = replace_once(
    text,
    '        "cal_kwic_dialect",\n',
