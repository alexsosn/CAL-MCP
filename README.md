...
| Citations | **Implemented** | `cal_external_citation_dialects`, `cal_external_citation_sources`, and `cal_external_citations`: explicit one-request discovery and lexical citation retrieval for CAL sources cited in the database but not available as full online texts |
| Targum | **Implemented** | `cal_targum_parallel`, `cal_targum_concordance`, `cal_targum_hebrew_lemmas`, and `cal_targum_hebrew_reflexes`: bounded verse comparison, Targum-specific counts, and explicit two-step MT-Hebrew-to-Targumic-lemma research with provenance |
| Syriac | **Implemented** | `cal_syriac_texts`, `cal_syriac_missing_words`, and `cal_syriac_peshitta_parallel`: bounded category browsing, CAL-curated missing-headword lists, and one-verse MT/Peshitta comparison with provenance |

The server distinguishes between **CAL data returned by the upstream service** and **adapter metadata produced by CAL-MCP**.

See [`docs/tools/lexicon.md`](docs/tools/lexicon.md) for lexicon lookup, [`docs/tools/search.md`](docs/tools/search.md) for English gloss/citation-text search, [`docs/tools/texts.md`](docs/tools/texts.md) for bounded CAL text discovery/page retrieval, [`docs/tools/token-analysis.md`](docs/tools/token-analysis.md) for token-at-coordinate lexical analysis, [`docs/tools/concordance.md`](docs/tools/concordance.md) for bounded frequency/KWIC operations, [`docs/tools/bibliography.md`](docs/tools/bibliography.md) for bounded CAL bibliography search, [`docs/tools/external-citations.md`](docs/tools/external-citations.md) for CAL citations from cited-but-not-online texts, [`docs/tools/targum.md`](docs/tools/targum.md) for bounded CAL Targum Studies workflows, and [`docs/tools/syriac.md`](docs/tools/syriac.md) for bounded CAL Syriac Studies workflows. CAL-owned identifier semantics are documented in [`docs/concepts/cal-identifiers.md`](docs/concepts/cal-identifiers.md).
...
- [`docs/tools/external-citations.md`](docs/tools/external-citations.md) — explicit dialect/source discovery and ordered lexical citations for CAL sources not available as full online texts.
- [`docs/tools/targum.md`](docs/tools/targum.md) — bounded parallel-verse comparison, Targum concordance, MT Hebrew lemma discovery/reflexes, failures, and provenance.
- [`docs/tools/syriac.md`](docs/tools/syriac.md) — bounded Syriac text-category browsing, CAL-curated missing-headword lists, MT/Peshitta verse comparison, failures, and provenance.
...
- live `cal_external_citation_dialects`, `cal_external_citation_sources`, and `cal_external_citations` tools with explicit caller-controlled dialect/source stages, one-request bounds, preserved external-source references, and no online-text or lexical-entry expansion;
- live `cal_targum_parallel`, `cal_targum_concordance`, `cal_targum_hebrew_lemmas`, and `cal_targum_hebrew_reflexes` tools with one-request operation bounds, exact CAL source order/labels, explicit zero/not-found semantics, and no hidden source/example/verse traversal;
- live `cal_syriac_texts`, `cal_syriac_missing_words`, and `cal_syriac_peshitta_parallel` tools with one-request bounds, descriptive public category slugs, explicit text/group navigation, CAL's *A Syriac Lexicon* comparison semantics, and no hidden category/entry/verse traversal;
