...
| Lexicon | `cal_lexicon_lookup`, `cal_lexicon_browse`, `cal_lexicon_citation_context` |
...
Normal CI is offline and uses reduced semantic fixtures. CAL-MCP does not expose cache warming, background refresh, persistent storage, automatic corpus traversal, or a generic invented pagination layer. `cal_lexicon_browse` returns at most one CAL browse page per call and exposes a validated `next_continuation` for a later explicit call rather than traversing automatically. Lexicon citation context, KWIC full context, and text-line comments/translations are likewise available only as separate explicit calls over typed selectors/coordinates returned by prior operations; parent lexicon/text/KWIC calls do not prefetch them.
