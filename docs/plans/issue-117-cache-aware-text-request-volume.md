...
A pre-review scope check then found two executable descriptions named by the committed research (`cal_text_catalogue` and `cal_text_search`) still retained unconditional exact-request wording. Focused test-only head `146058c0b4300f6f41a0b66fb06424135f72e3f7` passed dependency/environment validation, Ruff lint, Ruff format, and strict mypy in both matrices; deterministic pytest produced 753 passes and exactly two failures, one for each stale description, with all previously corrected wording and the cache invariant green. The minimal follow-up changed only those two docstrings.