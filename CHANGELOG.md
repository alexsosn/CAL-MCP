...
- text catalogue/topic discovery, one-page retrieval (including subdivided texts, whose `subtext_id` must be passed exactly as CAL returns it; each line keeps CAL's display coordinate and comment link), explicit text-information metadata, and explicit line comments/translations via `cal_text_line_comments`;
