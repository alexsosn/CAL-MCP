...
v0.1.0 freezes **33 public tools** across these research families:

- deterministic conversion from supported Aramaic-script/Unicode inputs to CAL code;
- lexicon lookup plus explicit linked-citation full context via `cal_lexicon_citation_context`;
...
- The release pipeline builds wheel and sdist once, validates both distributions in fresh virtual environments, launches each installed `cal-mcp` entry point over stdio, and checks version + the frozen 33-tool schema without contacting CAL.
